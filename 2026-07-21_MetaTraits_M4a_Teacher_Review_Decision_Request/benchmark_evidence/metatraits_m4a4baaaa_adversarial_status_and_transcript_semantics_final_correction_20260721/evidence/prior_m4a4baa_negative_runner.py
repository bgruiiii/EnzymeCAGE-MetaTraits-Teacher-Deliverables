#!/usr/bin/env python3
"""Execute all 41 inherited and 12 M4a-4BAA negative tests on temporary copies."""
...
def mutate(module,test_id,root):
 ...
 if test_id in {"N01","N02","N03","N04","N05"}:
  ...
  elif test_id=="N51": mutate_csv(root/"logs/validator_negative_tests.csv",lambda rows:rows[0].__setitem__("result","FAIL")); changed.append("logs/validator_negative_tests.csv")
 ...
 with tempfile.TemporaryDirectory(prefix="m4a4baa_negative_") as td:
  ...
  for tid,mutation,expected in module.NEGATIVE_CASES:
   if tid=="N42":
    ...
    fixture=base/tid
    if tid in {"N27","N29","N30","N43","N44","N45","N46"}: fixture.mkdir()
    elif tid in {"N31","N47"}: pass
    elif tid in {"N49","N50","N51","N52","N53"}: shutil.copytree(source,fixture,copy_function=shutil.copy2)
