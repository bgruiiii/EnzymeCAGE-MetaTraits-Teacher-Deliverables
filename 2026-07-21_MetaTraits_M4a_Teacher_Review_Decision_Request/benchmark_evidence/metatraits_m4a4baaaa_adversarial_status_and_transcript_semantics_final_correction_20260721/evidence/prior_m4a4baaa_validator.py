#!/usr/bin/env python3
"""Portable standard-library M4a-4BAAA validator and public fixture schema."""
...
CORRECTION_TOKEN="M4A4BAAA_READY_COMPLETE_STATUS_AND_TEST_EVIDENCE_FOR_LOCAL_AUDIT"
...
READY_FILES=frozenset(BAA_READY_FILES|BAAA_ADDED)
...
def validate_correction_report(root):
 text=(root/"M4A4BAAA_CORRECTION_REPORT.md").read_text(encoding="utf-8")
 expected="# M4a-4BAAA Complete Status And Test-Evidence Semantics Final Correction\n\nMACHINE_CHECKABLE_BOUNDARIES_BEGIN\n"+REPORT_BOUNDARY_BLOCK+"\nMACHINE_CHECKABLE_BOUNDARIES_END\n\nIndependent local audit remains the next gate; this correction does not claim formal M4a closure.\n"
 if text!=expected: fail("correction_report_semantics","exact report")

def validate_correction_provenance(root):
 cp=strict_json(root/"outputs/correction_provenance.json")
 keys={"added_path_count","authorized_replacement_count","benchmark_streams","captured_packaged_validation","checkpoint_loaded","coexistence","coexistence_pass","correction_stage","cross_consistent_negative_tests","dependency_changed","deterministic_tar_reconstruction","gpu_queried_or_used","immutable_path_count","m4b_authorized","m4c_authorized","model_modified_or_trained","network_api_called","positive_status_fixtures","prediction_executed","preserved_historical_copy_count","prior_identity_bytes","prior_identity_sha256","prior_manifest_bytes","prior_manifest_sha256","prior_tar_bytes","prior_tar_sha256","reverse_negative_tests","reverse_packaged_validation","reverse_positive_status_fixtures","source_or_overlay_modified","source_tree_unchanged_by_test_runners","teacher_pending","test_evidence_identities","torch_imported","warning_classification"}
 ...
 expected={"correction_stage":"M4a-4BAAA","prior_tar_bytes":2048000,"prior_tar_sha256":"1d2e80b594a76a2a8d22024c86ca978c569c07f50184a4e2948994a6fc43ac1e","prior_identity_bytes":2976,"prior_identity_sha256":"cbcbcd3a6c51669587fa8d5b8798cc5be2e965d18de4b2f7fdc4881fccdd0e51","prior_manifest_bytes":6680,"prior_manifest_sha256":"e34133912e35cc306aa747d9bea46aa0883ceda4dc264a9f24de5b777a6eb9d5","immutable_path_count":52,"authorized_replacement_count":14,"preserved_historical_copy_count":5,"added_path_count":7,"network_api_called":False,"dependency_changed":False,"source_or_overlay_modified":False,"model_modified_or_trained":False,"gpu_queried_or_used":False,"torch_imported":False,"checkpoint_loaded":False,"prediction_executed":False,"m4b_authorized":False,"m4c_authorized":False,"coexistence":"OPEN/NOT_ESTABLISHED","coexistence_pass":False,"teacher_pending":pending,"positive_status_fixtures":"6/6_PASS","cross_consistent_negative_tests":"68/68_PASS",**finals}
...
def validate_validator_results(root):
 ...
 keys={"actual_status_positive","actual_status_token","captured_evidence_validation","captured_packaged_validation","coexistence","coexistence_pass","cross_consistent_negative_tests","deterministic_tar_reconstruction","m4b_authorized","m4c_authorized","negative_tests_skipped","positive_status_fixtures","reverse_negative_tests","reverse_packaged_validation","reverse_positive_status_fixtures","source_tree_unchanged_by_test_runners","teacher_pending","test_evidence_identities","validator_status_dispatch"}
 ...
 expected={"actual_status_positive":"PASS","actual_status_token":READY,"captured_evidence_validation":"PASS","captured_packaged_validation":"PASS","coexistence":"OPEN/NOT_ESTABLISHED","coexistence_pass":False,"m4b_authorized":False,"m4c_authorized":False,"teacher_pending":pending,"validator_status_dispatch":True,"positive_status_fixtures":"6/6_PASS","cross_consistent_negative_tests":"68/68_PASS","negative_tests_skipped":[],**finals}
...
def parse_positive_transcript(text,rows):
 ...
 headers=[x for x in lines if re.match(r"F[0-9]{2}\t",x)]
 if len(headers)!=len(rows): fail("transcript_cross_consistency","positive count")
 pattern=re.compile(r"(F[0-9]{2})\tstatus=([^\t]+)\texit=([^\t]+)\tstderr_empty=([^\t]+)\ttoken_count=([^\t]+)\tdefining_condition=([^\t]+)\tresult=([^\t]+)")
 for line,row in zip(headers,rows):
...
def parse_negative_transcript(text,rows):
 ...
 headers=[(i,x) for i,x in enumerate(lines) if re.match(r"N[0-9]{2}\t",x)]
 if len(headers)!=len(rows): fail("transcript_cross_consistency","negative count")
 ...
 for n,((idx,line),row) in enumerate(zip(headers,rows)):
  m=pattern.fullmatch(line)
  ...
  if not any(x.startswith("M4A4BA_VALIDATION_ERROR:"+row["observed_reason"]+":") or x=="M4A4BA_VALIDATION_ERROR:"+row["observed_reason"] for x in lines[idx+1:end]): fail("transcript_cross_consistency","reason:"+row["test_id"])
...
def validate_ready(root,args):
 ...
 validate_fixed_ledger(root,args); rows,evidence,_=validate_request_calls(root); validate_statistics(root,rows); ledger=validate_preload_source(root,evidence); validate_memory(root); validate_pyg(root,ledger); validate_warnings(root); validate_inheritance(root,args.validation_phase); validate_baa_inheritance(root,args.validation_phase); validate_baaa_inheritance(root,args.validation_phase); cp=validate_correction_provenance(root); validate_correction_report(root)
...
def validate_blocked_gpu(root,args):
 ...
 if not isinstance(b,dict) or set(b)!={"type","stage","message","code"} or b.get("type")!="ImmediateGpuGateBlocker" or b.get("stage")!="immediate_gpu_gate" or b.get("code")!="GPU_FREE_MEMORY_BELOW_THRESHOLD" or not str(b.get("message","")).strip(): fail("gpu_blocker_schema","mismatch")
 observed=[]; previous=0
 for index,row in enumerate(rr,1):
  try:
   if row["sample"]!=str(index) or not UTC_RE.fullmatch(row["utc_timestamp"]) or not re.fullmatch(r"[1-9][0-9]*",row["monotonic_ns"]): fail("gpu_sample_semantics","index/time")
...
def validate_blocked_provenance(root,args):
 ...
 for item in rejected:
  ...
  if not inside: outside.append(item)
 if a==z and not outside: fail("provenance_blocker_semantics","no real blocker")
...
def build_status_fixture(target,status,validator_source,ready_source=None):
 ...
 if status in (READY,BLOCKED_PACKAGING):
  ...
  for rel in RESULT_FILES|(BAAA_ADDED if status==BLOCKED_PACKAGING else frozenset()):
   p=target/rel
   ...
   for phase,file in (("warmups","warmup_calls.csv"),("sequential","sequential_100_calls.csv"),("concurrency_2","concurrency_2_100_calls.csv"),("concurrency_4","concurrency_4_100_calls.csv")):
    ...
    with (target/"evidence"/file).open("w",newline="") as f:
     ...
     if status==INCOMPLETE and phase=="warmups":
      row={k:"" for k in CALL_HEADER}; row.update({"phase":phase,"global_request_id":"warmups-001","call_index":1,"start_utc":"2026-07-20T00:00:00Z","start_ns":1,"end_utc":"2026-07-20T00:00:00.001Z","end_ns":1000001,"latency_ms":1,"success":"false","exception_type":"IntegrityFailure","exception_message":"fixture integrity failure","integrity_pass":"false","integrity_failure_reason":"fixture","returned_count":0}); rows=[row]; w.writerow(row)
     if status==BLOCKED_PROVENANCE and phase=="warmups":
      row={k:"" for k in CALL_HEADER}; row.update({"phase":phase,"global_request_id":"warmups-001","call_index":1,"start_utc":"2026-07-20T00:00:00Z","start_ns":1,"end_utc":"2026-07-20T00:00:00.001Z","end_ns":1000001,"latency_ms":1,"success":"true","integrity_pass":"true","returned_count":100,"response_sha256":"1"*64,"uid_order_sha256":"2"*64,"evidence_hash":"3"*64,"max_abs_score_difference_from_first_sequential":0,"max_abs_ci_difference_from_first_sequential":0}); rows=[row]; w.writerow(row)
