 ...
 if baaa.exists():
  ...
  for q in x.get("files",[]):
   ...
   if rel in changed and rel!="MANIFEST.sha256" and (root/rel).exists():
    ...
    if q.get("classification")=="immutable": q["prior_bytes"],q["prior_sha256"]=n,h
    touched=True
  if touched: dump(baaa,x); changed.add("evidence/inherited_m4a4baa_file_identity.json")
 baaaa=root/"evidence/inherited_m4a4baaa_file_identity.json"
 if baaaa.exists():
  x=load(baaaa); touched=False
  for q in x.get("files",[]):
   rel=q["path"]
   if rel in changed and rel!="MANIFEST.sha256" and (root/rel).exists():
    n,h=ident(root/rel); q["new_bytes"],q["new_sha256"]=n,h
...
def mutate(module,test_id,root):
 ...
 if test_id in {"N01","N02","N03","N04","N05"}:
  ...
  elif test_id=="N65": (root/"M4A4BAAAA_CORRECTION_REPORT.md").write_text((root/"M4A4BAAAA_CORRECTION_REPORT.md").read_text()+"coexistence PASS and M4b authorized and M4a closed\n"); changed.append("M4A4BAAAA_CORRECTION_REPORT.md")
  ...
  module.build_status_fixture(root,module.INCOMPLETE,root.parent/"bootstrap/validate_m4a4b_formal_gpu_benchmark.py")
  wrong={"type":"PredictionFailure","stage":"warmups","message":"prediction failed during model forward call","code":"PREDICTION_FAILURE"}
  j("outputs/benchmark_provenance.json",lambda x:x.__setitem__("first_blocker",wrong)); j("outputs/benchmark_progress.json",lambda x:x.__setitem__("first_blocker",wrong))
 elif test_id=="N70":
  module.build_status_fixture(root,module.BLOCKED_PROVENANCE,root.parent/"bootstrap/validate_m4a4b_formal_gpu_benchmark.py")
  j("evidence/imported_enzymecage_modules.json",lambda x:x[0].__setitem__("realpath",module.CODE_ROOT+"/enzymecage/model.py"))
 elif test_id=="N71":
  module.build_status_fixture(root,module.BLOCKED_GPU,root.parent/"bootstrap/validate_m4a4b_formal_gpu_benchmark.py")
  for rel,key in (("outputs/benchmark_provenance.json","first_blocker"),("outputs/benchmark_progress.json","first_blocker"),("outputs/gpu_selection_or_block.json","blocker")):
   j(rel,lambda x,k=key:x[k].__setitem__("message","observed free memory is above threshold and safe to launch"))
 elif test_id=="N72":
  phase="packaged"; p=root/"logs/status_fixture_tests.stdout.txt"; p.write_text(p.read_text().replace("summary=6/6_PASS\n","CONTRADICTORY validation-error line\nsummary=6/6_PASS\n"),encoding="utf-8"); changed.append("logs/status_fixture_tests.stdout.txt")
  val={"bytes":ident(root/"logs/status_fixture_tests.stdout.txt")[0],"sha256":ident(root/"logs/status_fixture_tests.stdout.txt")[1]}
  for out in ("outputs/validator_results.json","outputs/correction_provenance.json"):
   x=load(root/out); x["test_evidence_identities"]["positive_stdout"]=val; dump(root/out,x); changed.append(out)
 elif test_id=="N73":
  mutate_csv(root/"evidence/sequential_100_calls.csv",lambda rows:rows[0].__setitem__("start_utc","2026-07-20 13:36:54Z")); changed.append("evidence/sequential_100_calls.csv")
 ...
 with tempfile.TemporaryDirectory(prefix="m4a4baa_negative_") as td:
  ...
  for tid,mutation,expected in module.NEGATIVE_CASES:
   if tid=="N42":
    ...
    fixture=base/tid
    if tid in {"N27","N29","N30","N43","N44","N45","N46","N54","N55","N56","N62","N63","N64","N69","N70","N71"}: fixture.mkdir()
    elif tid in {"N31","N47","N59","N60","N61"}: pass
    elif tid in {"N49","N50","N51","N52","N53","N57","N58","N65","N66","N67","N68","N72"}: shutil.copytree(source,fixture,copy_function=shutil.copy2)
